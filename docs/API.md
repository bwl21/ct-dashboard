...
## 📅 Auslaufende Termine API

### Übersicht
Die Auslaufende Termine API ermöglicht das Abrufen und Verwalten von Terminserien, die bald ablaufen oder bereits abgelaufen sind.
...
const filteredData = computed(() => {
  ...
  if (filters.statusFilter) {
    result = result.filter(item => 
      getItemStatus(item) === filters.statusFilter
    );
  }
